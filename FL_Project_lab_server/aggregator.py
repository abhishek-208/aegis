...
# --- === AEGIS REPUTATION STATE === ---
# Persistent per-client EMA of cosine penalty across rounds.
# Key: client_id, Value: float (EMA of cosine penalty, initialised to 0.0).
AEGIS_REPUTATION = {}

def reset_aegis_reputation():
    """Clears Aegis reputation history. Call before each experiment."""
    global AEGIS_REPUTATION
    AEGIS_REPUTATION = {}
...
def aegis(updates, current_round=None, 
    ...
    # --- Step 5b: Cross-Round Reputation Penalty (ALIE Defense) ---
    # R_k is an EMA of the client's historical cosine penalties.
    # Updated each round: R_k^{t+1} = γ * R_k^t + (1-γ) * P_k^t
    # Only approved clients are updated (rejected clients leave R_k unchanged).
    global AEGIS_REPUTATION

    reputation_penalties = torch.zeros(len(approved_indices), device=DEVICE)

    if config.REPUTATION_ENABLED:
        for idx_in_approved, idx_in_updates in enumerate(approved_indices.tolist()):
            client_id = all_client_ids[idx_in_updates]
            prev_reputation = AEGIS_REPUTATION.get(client_id, 0.0)
            reputation_penalties[idx_in_approved] = prev_reputation

            current_penalty = approved_cosine_penalties[idx_in_approved].item()
            new_reputation = (config.REPUTATION_DECAY * prev_reputation +
                              (1.0 - config.REPUTATION_DECAY) * current_penalty)
...
def bulyan(updates, fraction_byzantine, **kwargs):
    ...
    if n < 4 * f + 3:
        print(f"    > Bulyan: Constraint violated (n={n}, f={f}, need n>={4*f+3}). Skipping round.")
        return None, None
